...
GtOctreeLoader::GtOctreeLoader(const std::string &world_name, double resolution) : package_path(ros::package::getPath("vpp_learning_ros")),
  ...
  gt_pcl(new pcl::PointCloud<pcl::PointXYZ>()), gt_clusters(new std::vector<pcl::PointIndices>()), final_fruit_keys(new std::vector<octomap::KeySet>())
{
  ...
  YAML::Node plant_list = YAML::LoadFile(package_path + "/gt/world_plant_locations/" + world_name + "_plants.yaml");
...
void GtOctreeLoader::loadPlantTrees(const std::string &name, const std::string &resolution_str, size_t num_fruits)
{
  ...
  const std::string path = package_path + "/gt/plant_files/individual_fruits/" + name + "/";
